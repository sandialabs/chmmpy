...
class TourHMM_Default(HMMBase):
    ...
    def load_data(self, *, data=None, filename=None):
        ...
        for i,name in data['CityNames'].items():
            if name == data['Home']:
                alldata.home_city = i
                break
        assert alldata.home_city is not None, "The name of the home city must match one of the city names"
    ...
    def run_training_simulations(self, seed=None, n=None, debug=False, return_observations=False):
        ...
        for n in range(nruns):
            ...
            while all_cities[-1] != self.data.homeagain_city:
                random.shuffle(all_cities)                                  # Order the tour, which will end when we reach home again, or when we run out of money
            next_city_index = 0
            ...
            while i<Tmax:
                ...
                while next_city is None:
                    if city == self.data.home_city:
                        ...
                        next_city = city
                        costs += self.data.Costs[curr,city]
                    elif (curr, city) in self.data.Costs and costs + self.data.Costs[curr,city] + self.data.Costs[city,self.data.homeagain_city] <= budget:
                        next_city = city
                        costs += self.data.Costs[curr,city]
                    else:
                        assert next_city_index < len(all_cities)
    ...
    def print_ip_results(self, M):
            ...
            flag=False
            for j in J:
                print("z",j,-1,pe.value(M.z[j,-1]))
                for t in range(self.data.Tmax):
                    print("z",j,t,pe.value(M.z[j,t]))
...
class TourHMM_City3(TourHMM_Default):
    ...
    def create_ip(self, *, observation_index, emission_probs, data):
        M = TourHMM_Default.create_ip(self, observation_index=observation_index, emission_probs=emission_probs, data=data)
